...
if [ -z "$SERVICE" ] || [ -z "$BACKUP_DIR" ]; then
  ...
  echo "  services: django, payment-service, notifications-service, auth-service, market-service, support-service" >&2
  echo "  e.g. $0 payment-service backups/20260815_030000" >&2
  exit 1
fi

case "$SERVICE" in
  django)                DB_PATH=/app/db.sqlite3;            DB_FILE=db.sqlite3 ;;
